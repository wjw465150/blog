...
webimPopup.getSender=function(arr)
{
    if(arr && arr.length && arr.length>0)
    {
        ...
        for(var i=0;i<arr.length;i++)
	    {
	        ...
	        //fanwx 081030 修改
		    //l.push('&nbsp;<a href="http://blog.csdn.net/lldwolf/archive/2008/04/17/2299622.aspx#" onclick="webimPopup.showChat(\''+ arr[i] +'\');return false;">'+ arr[i] +'</a><br/>');
            l.push('&nbsp;<a href="'http://blog.csdn.net/lldwolf/archive/2008/04/17/+csdn_im_defaultDomain+'/messages/'+escape(arr[i])+'.ashx" target="_blank" onclick="webimPopup.hideItem(\''+i+'\')">'+ arr[i] +'</a></div>');
	    }
