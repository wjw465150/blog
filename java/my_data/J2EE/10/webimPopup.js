...
webimPopup.getSender=function(arr)
{
    if(arr && arr.length && arr.length>0)
    {
        ...
        for(var i=0;i<arr.length;i++)
	    {
	        ...
	        //fanwx 081030 修改
		    //l.push('&nbsp;<a href="http://blog.csdn.net/java2000_net/archive/2008/02/25/2119298.aspx#" onclick="webimPopup.showChat(\''+ arr[i] +'\');return false;">'+ arr[i] +'</a><br/>');
            l.push('&nbsp;<a href="'http://blog.csdn.net/java2000_net/archive/2008/02/25/+csdn_im_defaultDomain+'/messages/'+escape(arr[i])+'.ashx" target="_blank">'+ arr[i] +'</a><br/>');
	    }
	    return l.join('');
    }
}
