...
webImForm.prototype.show=function()
{
    ...
    l.push('    <a href="http://blog.csdn.net/java2000_net/archive/2008/09/17/2940219.aspx#" onclick="webImForm.minimize(\''+ this.divId +'\',document.getElementById(\''+ this.divId +'Minimize\'));return false;">');
    l.push('        <img id="'+this.divId+'Minimize" src="'+ fpath +'csdnim080507verpic4.gif" class="founctionpic2" alt="minimize"/>');
    l.push('    </a>');
    l.push('    <a href="http://blog.csdn.net/java2000_net/archive/2008/09/17/2940219.aspx#" onclick="webImForm.close(\'' + this.divId + '\');return false;">');
...
webimPopup.getSender=function(arr)
{
    if(arr && arr.length && arr.length>0)
    {
        ...
        for(var i=0;i<arr.length;i++)
	    {
	        ...
	        //fanwx 081030 修改
		    //l.push('&nbsp;<a href="http://blog.csdn.net/java2000_net/archive/2008/09/17/2940219.aspx#" onclick="webimPopup.showChat(\''+ arr[i] +'\');return false;">'+ arr[i] +'</a><br/>');
            l.push('&nbsp;<a href="'http://blog.csdn.net/java2000_net/archive/2008/09/17/+csdn_im_defaultDomain+'/messages/'+encodeURIComponent(arr[i])+'.ashx" target="_blank" onclick="webimPopup.hideItem(\''+i+'\')">'+ arr[i] +'</a></div>');
	    }
	    l.push('</div>');
